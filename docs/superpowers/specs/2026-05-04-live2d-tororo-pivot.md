...
| 维度 | 选择 |
|---|---|
| 模型 | Live2D 公式 sample **Tororo**（白色短毛猫；Cubism **2** 老格式 .moc/.mtn） |
| 渲染 SDK | `pixi.js@7` + `pixi-live2d-display@0.4` + `live2d.min.js`（Cubism 2 runtime, dylanNew mirror）+ `live2dcubismcore.min.js`（Cubism 4 core，备未来用） |
...
| 反应（happy/sad/...） | Tororo 有 1 idle motion + 8 个无名 tap motion（`mtn/01.mtn`–`mtn/08.mtn`）。降级方案：保留 detectExpression 文本分类，'happy' → 随机 tap motion；其他 expression 也可映射到不同 tap index | `motionMapper.ts` + `MotionController.playReaction` |
...
### Step 1 — 装依赖 + 放静态资源（**已完成**，2026-05-04）

- ✅ 资源已下载：
  - `src/renderer/public/cubism/live2d.min.js` (129 KB) — Cubism 2 runtime, [dylanNew/live2d](https://github.com/dylanNew/live2d) mirror
  - `src/renderer/public/cubism/live2dcubismcore.min.js` (207 KB) — Cubism 4 core, 官方 cubism.live2d.com
  - `src/renderer/public/model/tororo/` (586 KB) — Tororo 模型 .moc / .mtn 全套，从 [akyuu-cn/thermal-cat](https://github.com/akyuu-cn/thermal-cat) 镜像
- ⏳ 待做：`npm install pixi.js@^7.4 pixi-live2d-display@^0.4`
- ⏳ 待做：`.gitignore` 加 `src/renderer/public/cubism/*.js` 和 `src/renderer/public/model/**`
- ⏳ 待做：`index.html` 在 `<script type="module" src="./main.tsx">` 之前加：
  ```html
  <script src="./cubism/live2d.min.js"></script>
  <script src="./cubism/live2dcubismcore.min.js"></script>
  ```
- commit: "chore: add live2d cubism runtimes and tororo sample model"

### Step 2 — Live2DStage 最小可显示 + 删 VRM

- 写 `src/renderer/stage/Live2DStage.ts`：构造一个 PIXI.Application 接管 canvas，`Live2DModel.from('./model/tororo/index.json')`（Tororo 用非标准 `index.json` 文件名，pixi-live2d-display 通过 URL 直接吃 JSON），stage.addChild(model)，居中缩放，dispose 清理 ticker + app
